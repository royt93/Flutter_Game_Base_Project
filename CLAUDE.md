...
- **`app_translations.dart`** — `AppTranslations`, 2 seed locales (`en`/`vi`), 6 base keys (`app_name`, `settings`, `language`, `sound`, `ok`, `cancel`). Copy this file's one-`Map`-per-locale pattern when a consumer needs more languages/keys.
