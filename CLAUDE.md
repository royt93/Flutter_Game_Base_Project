...
- **`utils/trusted_clock.dart`** — `TrustedClockService`/`ClockSample`/`ClockJudgement`, a stricter wall-clock-vs-monotonic drift check (backed by `StorageKeys.trustedClockBaselineMs`/`trustedClockPrevWallMs`/`trustedClockPrevMonotonicMs`) for time-gated systems that need more than `ClampedClock`'s simple never-goes-backward guarantee. Reach for this instead of plain `nowMsClamped()` specifically for a ONE-SHOT, high-value payout (offline earnings, a big claim) rather than a repeating one (a daily streak, energy regen ticking up gradually) — `nowMsClamped()`'s watermark advances to whatever wall-clock value it's handed, so winding the clock forward once, claiming, then winding it back still pays out that one fraudulent claim (the watermark then just stays stuck at the future value, locking further HONEST claims until real time catches up — a self-inflicted lock, not a fix). `TrustedClockService` classifies that same forward jump as `ClockJudgement.suspiciousForwardJump`, refuses to advance its baseline for it (so the fraudulent claim pays out 0), and — unlike `ClampedClock` — recovers normally once an honest sample follows instead of staying locked. `OfflineProgressionService.trustedClock` (ENH-86, optional, default `null` = unchanged `nowMsClamped()` behavior) is the reference integration — a consumer reads `TrustedClockService.lastJudgement` itself after a rejected claim to log/alert, this layer doesn't duplicate that reporting.
