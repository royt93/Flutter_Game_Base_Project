...
void main() {
  group('Wave 20.2 — 200 màn / 10 thế giới', () {
    test('kLevelCount = 200, kWorlds = 10', () {
      expect(kLevelCount, 200);
  ...
  group('Wave 20.2 — weave cơ chế mới W9-10', () {
    ...
    test('weave dead-zone W9-10 ({159,177}): clearJelly + corner pattern', () {
      for (final idx in {159, 177}) {
        final lv = kLevels[idx - 1];
        expect(lv.objective, ObjectiveType.clearJelly,
            reason: 'dead-zone weave vào màn clearJelly');
        expect(lv.jelly, JellyPattern.corner,
    ...
    test('weave order W9-10 ({157,187}): order objective', () {
      for (final idx in {157, 187}) {
        final lv = kLevels[idx - 1];
        expect(lv.objective, ObjectiveType.order,
            reason: 'order weave vào màn score');
        expect(lv.orders, isNotEmpty);
        expect(lv.orders!.length, 3, reason: '3 màu mục tiêu'); // ignore: unnecessary_non_null_assertion
      }
    });
    ...
    test('weave bomb W9-10 ({151,199}): score + bonusMoves', () {
      for (final idx in {151, 199}) {
        ...
        expect(lv.moves, greaterThan(17),
  ...
  group('Wave 20.2 — winnability layouts (fill đầy + hasMove)', () {
    test('bàn 163 fill đầy 100% ô chơi, có nước đi', () => _testLayoutFill(163));
    test('bàn 175 fill đầy 100% ô chơi, có nước đi', () => _testLayoutFill(175));
    test('bàn 193 fill đầy 100% ô chơi, có nước đi', () => _testLayoutFill(193));
  });
  ...
  group('Wave 20.2 — winnability horizontal matchability (M1 fix)', () {
    ...
    test('bàn 163 có hàng ngang ≥3 ô chơi liên tiếp', () => _testHorizontalMatch(163));
    test('bàn 175 có hàng ngang ≥3 ô chơi liên tiếp', () => _testHorizontalMatch(175));
    test('bàn 193 có hàng ngang ≥3 ô chơi liên tiếp', () => _testHorizontalMatch(193));
  });
  ...
  group('Wave 20.2 — đường cong độ khó', () {
    test('màn 151-200 không có objective invalid (endless/boss/soda)', () {
      for (int i = 150; i < 200; i++) {
        expect(
          kLevels[i].objective,
          isNot(anyOf(
              ObjectiveType.endless, ObjectiveType.boss, ObjectiveType.soda)),
        );
      }
    });
    ...
    test('moves W9-10 trong khoảng hợp lý [17, 35]', () {
      for (int i = 150; i < 200; i++) {
        ...
        expect(lv.moves, inInclusiveRange(17, 35),
            reason: 'màn ${lv.index} moves=${lv.moves} nằm ngoài khoảng');
      }
    });

    test('targetScore score/timeAttack W9-10 > 0 (order dùng orders thay thế)', () {
      for (int i = 150; i < 200; i++) {
        final lv = kLevels[i];
        if (lv.objective == ObjectiveType.score ||
            lv.objective == ObjectiveType.timeAttack) {
          expect(lv.targetScore, greaterThan(0),
              reason: 'màn ${lv.index} (${lv.objective}) phải có targetScore > 0');
        }
      }
    });
  });
...
void _testLayoutFill(int levelIndex) {
  ...
  expect(playCells, greaterThan(40),
...
void _testHorizontalMatch(int levelIndex) {
  ...
  expect(found, isTrue,
      reason: 'màn $levelIndex phải có ít nhất 1 hàng với ≥3 ô chơi liên tiếp (horizontal match)');
}
