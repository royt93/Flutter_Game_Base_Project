...
Future<ProcessResult> _runCheck(List<String> args) => Process.run('dart', [
  'run',
  'tool/accessibility_audit_check.dart',
  ...args,
]);
    ...
    test(
      ...
      () async {
        ...
        expect(
          ...
          contains('No accessibility issues found.'),
        );
      },
  ...
  group('CLI: fixture --root synthetic bắt lỗi thật', () {
    ...
    setUp(() {
      tempDir = Directory.systemTemp.createTempSync(
        'accessibility_audit_check_test_',
      );
    });
    ...
    test(
      'widget lỗi (GestureDetector không Semantics) -> exit code 1, báo đúng file',
      () async {
        writeFile(
          'widgets/bad.dart',
    ...
    test(
      'widget hợp lệ -> exit code 0, không false fail',
      () async {
        writeFile(
          'widgets/good.dart',
    ...
    test(
      'baseline suppress đúng finding theo file+rule, có lý do',
      () async {
        writeFile(
          'widgets/bad.dart',
          "GestureDetector(onTap: () {}, child: Text('x'))",
    ...
    test(
      'không có file baseline -> vẫn chạy bình thường (baseline rỗng)',
      () async {
    ...
    test(
      '--minSeverity=error chỉ fail khi có lỗi mức error, bỏ qua warning',
      () async {
        writeFile(
          'widgets/warn_only.dart',
          "GestureDetector(onTap: () {}, child: Text('x'))",
        );
