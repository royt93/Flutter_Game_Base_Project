...
Future<ProcessResult> _runCheck(List<String> args) =>
    Process.run('dart', ['run', 'tool/dependency_sbom_check.dart', ...args]);
...
void main() {
  group('CLI: dart run tool/dependency_sbom_check.dart trên repo thật', () {
    ...
    test(
      '--minSeverity=medium bỏ qua finding low (dbus), exit code 0',
      () async {
...
''';
    ...
    test(
      'package không có LICENSE trong pubCache giả -> unknownLicense, exit 1',
      () async {
        writeFile('pubspec.lock', fakeLock);
        writeFile('pubspec.yaml', fakePubspec);
        final emptyPubCache = Directory('${tempDir.path}/empty_pub_cache')
          ..createSync();
    ...
    test(
      'dependency không pin version (wild_pkg:) -> unpinnedConstraint bị bắt',
      () async {
        writeFile('pubspec.lock', fakeLock);
        writeFile('pubspec.yaml', fakePubspec);
        final emptyPubCache = Directory('${tempDir.path}/empty_pub_cache')
    ...
    test(
      'package có LICENSE MIT thật trong pubCache giả -> không báo unknownLicense',
      () async {
        writeFile('pubspec.lock', fakeLock);
        writeFile(
          'pubspec.yaml',
        ...
        expect(
          ...
          contains('No dependency issues found.'),
        );
      },
      timeout: const Timeout(Duration(seconds: 30)),
    );
    ...
    test(
      'advisory khớp version đang dùng -> báo vulnerability, exit 1',
      () async {
        writeFile('pubspec.lock', fakeLock);
        writeFile(
          'pubspec.yaml',
    ...
    test(
      'suppression có owner/reason/expiresAtMs còn hạn -> ẩn đúng finding',
      () async {
        writeFile('pubspec.lock', fakeLock);
        writeFile(
          'pubspec.yaml',
    ...
    test(
      '--out= ghi ra file SBOM JSON hợp lệ, đọc lại parse được',
      () async {
        writeFile('pubspec.lock', fakeLock);
        writeFile('pubspec.yaml', fakePubspec);
        final emptyPubCache = Directory('${tempDir.path}/empty_pub_cache')
