...
void main() {
  ...
  group('CLI: schema hợp lệ -> sinh đúng file, code chạy được thật', () {
    test(
      'sinh model, dart run harness import trực tiếp cho ra kết quả đúng',
      () async {
...
''');
  ...
  group('CLI: schema hỏng -> KHÔNG sinh code, exit 1', () {
    test(
      'packName không phải identifier -> reject, không ghi file nào',
      () async {
        final schemaPath = '${tempDir.path}/schema.json';
        File(schemaPath).writeAsStringSync(
          jsonEncode({
  ...
  group(
    'CLI: --fixture/--secret gate signature/version trước khi sinh code',
    () {
      const secret = 'shared-secret';

      test(
        'fixture ký sai secret -> reject, không sinh file',
        () async {
          final schemaPath = '${tempDir.path}/schema.json';
          File(schemaPath).writeAsStringSync(
            jsonEncode({
      ...
      test(
        'fixture ký đúng, version hợp lệ -> verify OK rồi mới sinh code',
        () async {
          final schemaPath = '${tempDir.path}/schema.json';
          File(schemaPath).writeAsStringSync(
            jsonEncode({
