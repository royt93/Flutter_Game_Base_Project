...
Future<ProcessResult> _runCheck(List<String> args) => Process.run(
  'dart',
  ['run', 'tool/asset_license_check.dart', ...args],
);
...
void main() {
  group('CLI: dart run tool/asset_license_check.dart trên repo thật', () {
    test(
      ...
      () async {
        ...
        expect(result.stdout as String, contains('No asset license issues found.'));
  ...
  group('CLI: fixture --root synthetic bắt lỗi thật', () {
    ...
    setUp(() {
      tempDir = Directory.systemTemp.createTempSync('asset_license_check_test_');
    });
    ...
    test('asset không có entry manifest -> exit code 1, báo đúng path', () async {
    ...
    test('license bị cấm phân phối -> exit code 1, báo đúng disallowedLicense', () async {
    ...
    test('manifest có entry thừa (asset đã xoá) -> exit code 1, báo đúng stale', () async {
      await writeManifest({
        'schemaVersion': 1,
        'entries': [
          {
    ...
    test('asset + manifest khớp nhau hoàn toàn -> exit code 0 trên fixture riêng', () async {
    ...
    test('không có file manifest nào -> vẫn không crash, báo missing cho mọi asset', () async {
    ...
    test('.DS_Store và LICENSES.json tự thân không bị coi là asset cần khai', () async {
