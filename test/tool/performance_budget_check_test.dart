...
void main() {
  ...
  group('CLI: policy/baseline tuỳ chỉnh qua temp file', () {
    ...
    test(
      'thiếu baseline realDevice -> missingMeasurement, exit 1',
      () async {
    ...
    test(
      'budget host quá chặt so với thực đo -> budgetExceeded, exit 1',
      () async {
        final policyPath = '${tempDir.path}/policy.json';
        File(policyPath).writeAsStringSync(
          jsonEncode([
    ...
    test(
      'policy JSON có entry hỏng -> bỏ qua entry đó, không crash',
      () async {
        final policyPath = '${tempDir.path}/policy.json';
        File(policyPath).writeAsStringSync(
          jsonEncode([
    ...
    test('snapshot (không --device) ghi đúng 2 metric hostHeadless, giữ '
      ...
      expect(
        byName['object_pool_allocation_reduction_percent']!['value'],
        97.5,
      );
