...
void main() {
  group('LevelUpOverlayController: single level-up', () {
    test('phase đi đúng thứ tự xpFill -> levelPop -> rewardReveal -> idle', () async {
  ...
  group('LevelUpOverlayController: multi-level queue', () {
    ...
    test('onComplete chỉ gọi đúng 1 lần ở CUỐI toàn bộ queue, không phải mỗi level', () async {
      final controller = _controller();
      var completeCount = 0;
  ...
  group('LevelUpOverlayController: skip', () {
    test('skip() giữa chừng: nhảy thẳng về idle, onComplete gọi đúng 1 lần', () async {
      final controller = LevelUpOverlayController(
        xpFillDuration: const Duration(seconds: 10), // đủ dài để skip kịp giữa chừng
    ...
    test('skip() rồi gọi lại nhiều lần: không gọi onComplete trùng', () async {
      ...
      final future = controller.show([_event(2)], onComplete: () => completeCount++);
      await Future<void>.delayed(Duration.zero);
      controller.skip();
      controller.skip();
      controller.skip();
  ...
  group('LevelUpOverlayController: dispose', () {
    test('dispose() giữa chừng: không mutate phase thêm, không gọi onComplete', () async {
      final controller = LevelUpOverlayController(
      ...
      expect(controller.phase.value, phaseAtDispose); // không đổi thêm sau dispose
