...
void main() {
  ...
  group('detectPlatformCapabilities: device matrix (fake isWeb/targetPlatform)', () {
    ...
    test('Web (bất kể targetPlatform bên dưới): cả 4 capability đều false', () {
      final snapshot = detectPlatformCapabilities(
        isWeb: true,
        targetPlatform: TargetPlatform.android,
      );
    ...
    test('Windows: shader/notification/backgroundAudio true, haptics false', () {
      final snapshot = detectPlatformCapabilities(
        isWeb: false,
    ...
    test('không truyền gì: dùng đúng kIsWeb/defaultTargetPlatform thật của môi trường test', () {
      final snapshot = detectPlatformCapabilities();
  ...
  group('PlatformCapabilityRegistry: SSOT, cache, không platform channel', () {
    ...
    test('snapshot được tính 1 lần lúc construct, expose lại y hệt mỗi lần đọc', () {
    ...
    test('inject snapshot thẳng (override auto-detect) cho test/consumer tự quyết định', () {
    ...
    test('Get.put đúng instance, maybe tìm lại được', () {
      final registry = PlatformCapabilityRegistry(
        snapshot: detectPlatformCapabilities(isWeb: false, targetPlatform: TargetPlatform.iOS),
      );
  ...
  group('PlatformCapabilityRegistry.withFallback: chính sách fallback rõ ràng, không crash', () {
    test('supported = true: chạy nhánh ifSupported', () {
      final registry = PlatformCapabilityRegistry(
        snapshot: detectPlatformCapabilities(isWeb: false, targetPlatform: TargetPlatform.android),
      );
    ...
    test('ifSupported ném lỗi không bị withFallback nuốt (không che giấu bug thật)', () {
      final registry = PlatformCapabilityRegistry(
        snapshot: detectPlatformCapabilities(isWeb: false, targetPlatform: TargetPlatform.android),
      );
