...
void main() {
  ...
  group('consent gate', () {
    test('chưa có ConsentStateService -> default-deny, không forward', () {
      ...
      expect(
        sampler.auditSnapshot.droppedByReason[AnalyticsDropReason
            .consentNotGranted],
        1,
      );
    });
  ...
  group('deterministic sampling', () {
    test(
      'sampling rate 0.0 cho 1 event: luôn drop, các event khác không ảnh hưởng',
      () {
        _grantConsent();
        final inner = _RecordingAnalyticsProvider();
        ...
        expect(
          inner.calls.where((c) => c.key == 'important_event'),
          hasLength(5),
        );
        expect(
    ...
    test(
      'cùng session seed + cùng event name -> quyết định giống hệt mọi lần gọi',
      () {
        _grantConsent();
        final inner = _RecordingAnalyticsProvider();
        final sampler = PrivacyAwareAnalyticsSampler(
        ...
        expect(
          results.toSet(),
          hasLength(1),
        ); // luôn cùng 1 giá trị (all-true hoặc all-false)
      },
    );

    test(
      'session seed khác nhau -> có thể ra quyết định khác nhau (không cùng 1 coin flip cứng)',
      () {
        _grantConsent();
        final inner = _RecordingAnalyticsProvider();
  ...
  group('PII redaction qua SdkEventSchemaRegistry', () {
    test(
      'registry redact field pii trước khi tới provider, audit không tính rateLimited',
      () {
        _grantConsent();
    ...
    test(
      'registry reject (unknown event) -> drop, không forward, đúng audit reason',
      () {
        _grantConsent();
        final registry = SdkEventSchemaRegistry();
        final inner = _RecordingAnalyticsProvider();
  ...
  group('rate limit / backpressure', () {
    test(
      'vượt maxEventsPerWindow trong cùng window -> drop, không throw, không block',
      () {
        _grantConsent();
        final inner = _RecordingAnalyticsProvider();
        var now = 1000;
  ...
  group('auditSnapshot', () {
    test('totalDropped cộng đúng mọi reason, forwarded đếm đúng', () {
      ...
      expect(
        snapshot.droppedByReason[AnalyticsDropReason.consentNotGranted],
        2,
      );
    });
  });
