...
void main() {
  ...
  group('isKilled: giá trị remote hợp lệ', () {
    ...
    test('map đầy đủ {killed, reason, version} -> đọc đúng cả 3 field', () async {
      final config = await configWith({
        'kill_switch_shop': {'killed': true, 'reason': 'payment provider down', 'version': 3},
      });
      final controller = RemoteKillSwitchController(remoteConfig: config);
  ...
  group('asset default: chưa từng có remote hợp lệ', () {
    test('không có key remote, không có asset default -> killed=false', () async {
      final config = await configWith({});
      final controller = RemoteKillSwitchController(remoteConfig: config);

      expect(controller.isKilled('shop'), isFalse);
      expect(controller.states['shop']!.source, KillSwitchSource.assetDefault);
    });

    test('không có key remote, có asset default killed=true -> dùng asset default', () async {
      final config = await configWith({});
  ...
  group('PHÁT HIỆN THẬT: remote invalid không được mở lại feature đã bị kill', () {
    test(
      'remote hợp lệ killed=true trước, sau đó remote hỏng (sai type) -> '
      'vẫn giữ killed=true từ cache, KHÔNG revert về false',
      () async {
        ...
        expect(controller.isKilled('shop'), isTrue, reason: 'không được tự ý mở lại feature');
        expect(controller.states['shop']!.source, KillSwitchSource.cachedLastKnownGood);
      },
    );

    test('map remote thiếu field "killed" hoặc sai type -> bị từ chối hoàn toàn (không dùng)', () async {
      final config = await configWith({
    ...
    test('remote là số/string thô (không phải bool/map) -> bị từ chối, dùng asset default', () async {
  ...
  group('reactive: states cập nhật đúng cho widget đang watch', () {
    test('isKilled cập nhật states[featureId] ngay, nhiều feature độc lập nhau', () async {
      final config = await configWith({
    ...
    test('refreshAll cập nhật đủ mọi feature id truyền vào', () async {
      ...
      expect(controller.states.keys, containsAll(['shop', 'event', 'animation']));
  ...
  group('runIfEnabled: guard chống grant sai', () {
    ...
    test('feature không bị kill -> callback được gọi, trả đúng kết quả', () async {
      final config = await configWith({'kill_switch_shop': false});
      final controller = RemoteKillSwitchController(remoteConfig: config);
