...
void main() {
  group('scanAccessibility: reducedMotionCoverage', () {
    test('fixture cố tình lỗi: AnimationController không có reducedMotion -> bị bắt', () {
...
''',
    ...
    test('fixture hợp lệ: AnimationController có tham chiếu reducedMotion -> không false fail', () {
      final violations = scanAccessibility(
        fileContents: {
          'good.dart': '''
...
''',
    ...
    test('file không có AnimationController -> rule không áp dụng, không báo gì', () {
      final violations = scanAccessibility(
        fileContents: {'plain.dart': 'class Plain extends StatelessWidget {}'},
      );
      expect(violations, isEmpty);
    });
  });
  ...
  group('scanAccessibility: tapTargetSemantics', () {
    test('fixture cố tình lỗi: GestureDetector không có Semantics -> bị bắt', () {
      final violations = scanAccessibility(
        fileContents: {
          'bad_tap.dart': '''
...
''',
    ...
    test('fixture hợp lệ: GestureDetector có Semantics -> không false fail', () {
      final violations = scanAccessibility(
        fileContents: {
          'good_tap.dart': '''
Widget build(BuildContext context) {
...
''',
    ...
    test('fixture hợp lệ: dùng ExcludeSemantics (trang trí, có lý do rõ) -> không false fail', () {
      final violations = scanAccessibility(
        fileContents: {
          'decor_tap.dart': '''
Widget build(BuildContext context) {
...
''',
    ...
    test('InkWell cũng bị áp dụng rule y hệt GestureDetector', () {
      final violations = scanAccessibility(
        fileContents: {'bad_ink.dart': "InkWell(onTap: () {}, child: Text('x'))"},
      );
      expect(violations, hasLength(1));
      expect(violations.single.ruleId, AuditRuleId.tapTargetSemantics);
    });
  ...
  group('scanAccessibility: rtlDirectionalSafety', () {
    test('fixture cố tình lỗi: EdgeInsets.only(left:) -> bị bắt', () {
      final violations = scanAccessibility(
        fileContents: {'bad_rtl.dart': 'padding: const EdgeInsets.only(left: 8),'},
      );
      expect(violations, hasLength(1));
    ...
    test('fixture cố tình lỗi: Positioned(right:) -> bị bắt', () {
      final violations = scanAccessibility(
        fileContents: {'bad_pos.dart': 'Positioned(right: 0, top: 0, child: X())'},
      );
      expect(violations, hasLength(1));
      expect(violations.single.ruleId, AuditRuleId.rtlDirectionalSafety);
    });

    test('fixture hợp lệ: EdgeInsetsDirectional(start:) -> không false fail', () {
      final violations = scanAccessibility(
        fileContents: {'good_rtl.dart': 'padding: const EdgeInsetsDirectional.only(start: 8),'},
      );
      expect(violations, isEmpty);
    });

    test('fixture hợp lệ: chỉ dùng top:/bottom: -> không false fail (không phải trục ngang)', () {
      final violations = scanAccessibility(
        fileContents: {'good_vertical.dart': 'EdgeInsets.only(top: 8, bottom: 4)'},
      );
      expect(violations, isEmpty);
    });
...
''',
    ...
    test('code thật trên cùng dòng với // comment vẫn bị bắt (chỉ cắt phần sau //)', () {
      final violations = scanAccessibility(
        fileContents: {
          'mixed.dart': 'padding: const EdgeInsets.only(left: 8), // TODO: fix RTL sau',
        },
      );
      expect(violations, hasLength(1));
    });
  ...
  group('AuditBaseline: suppress có lý do', () {
    test('suppress đúng 1 (file, rule) cụ thể, không ảnh hưởng violation khác', () {
    ...
    test('entry thiếu reason (rỗng) bị bỏ qua hoàn toàn -> vẫn báo violation bình thường', () {
      final baseline = AuditBaseline.fromJson([
        {'file': 'legacy.dart', 'rule': 'tapTargetSemantics', 'reason': ''},
      ]);

      final violations = scanAccessibility(
        fileContents: {'legacy.dart': "GestureDetector(onTap: () {}, child: Text('x'))"},
        baseline: baseline,
      );

      expect(violations, hasLength(1), reason: 'reason rỗng -> baseline entry vô hiệu, không suppress được');
    });
    ...
    test('map rác lẫn trong list JSON không làm hỏng các entry hợp lệ khác', () {
      final baseline = AuditBaseline.fromJson([
        {'file': 'ok.dart', 'rule': 'rtlDirectionalSafety', 'reason': 'debug-only overlay'},
        'garbage',
        42,
        null,
...
''',
    ...
    test('file sạch hoàn toàn -> không có trong kết quả (không báo "clean" thừa)', () {
      final violations = scanAccessibility(
        fileContents: {
