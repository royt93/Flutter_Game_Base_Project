...
void main() {
  group('SeededRandom: determinism', () {
    test(
      'cùng seed cho đúng cùng chuỗi output (nextInt/nextDouble/nextBool)',
      () {
    ...
    test(
      'golden vector: seed cố định cho đúng chuỗi cố định (bắt regression thuật toán)',
      () {
        final r = SeededRandom(1);
  ...
  group('SeededRandom: shuffle/pick', () {
    ...
    test(
      'pick: bridge đúng sang weightedRandomPick, cùng seed cho cùng kết quả',
      () {
    ...
    test(
      ...
      () {
        final r = SeededRandom(1);
        final before = r.snapshot();
        expect(() => r.pick(['a', 'b'], [1.0, -1.0]), throwsArgumentError);
        expect(r.snapshot().state, before.state);
      },
    );

    test(
      'pick: implements Random nên dùng trực tiếp được với List.shuffle',
      () {
  ...
  group('SeededRandom: snapshot/restore', () {
    test(
      'snapshot rồi restore tiếp tục ĐÚNG chuỗi tiếp theo (không lặp lại, không nhảy cóc)',
      () {
    ...
    test(
      'RandomSnapshot.fromJson với dữ liệu hỏng/thiếu field throw FormatException',
      () {
        expect(
  ...
  group('CompiledWeightedTable', () {
    ...
    test('validate ngay lúc tạo — items rỗng throw', () {
      expect(() => CompiledWeightedTable<String>([], []), throwsArgumentError);
    });
    ...
    test(
      'pick() nhiều lần không re-validate — trọng số 0 không bao giờ được chọn',
      () {
    ...
    test(
      'cùng SeededRandom seed → CompiledWeightedTable.pick() deterministic',
      () {
  ...
  group('SeededRandomService: namespace streams', () {
    ...
    test(
      'cùng rootSeed + cùng namespace trên 2 service khác nhau → cùng chuỗi',
      () {
        final serviceA = SeededRandomService(555);
        final serviceB = SeededRandomService(555);
        final seqA = List.generate(
          10,
          (_) => serviceA.stream('loot').nextInt(1000),
        );
        final seqB = List.generate(
          10,
          (_) => serviceB.stream('loot').nextInt(1000),
        );
        expect(seqA, seqB);
      },
    );
    ...
    test(
      'snapshotAll/restoreSnapshots khôi phục đúng TỪNG namespace độc lập',
      () {
        ...
        final restored =
            SeededRandomService(
                999,
              ) // rootSeed khác cố ý — phải bị ghi đè bởi restore
