...
void main() {
  group('RetryPolicy.delayBeforeAttempt', () {
    ...
    test(
      'attempt 2, randomValue=0.5 (giữa): đúng bằng baseDelay, không jitter',
      () {
        expect(
    ...
    test(
      'jitter randomValue=0 -> thấp hơn base; randomValue=1 -> cao hơn base',
      () {
    ...
    test(
      'maxAttempts/jitterFraction biên hợp lệ không throw lúc construct',
      () {
        expect(
  ...
  group('RetryExecutor.run', () {
    ...
    test(
      'fail 2 lần rồi thành công lần 3: phục hồi đúng sau lỗi transient',
      () async {
        var calls = 0;
        final result = await run(() async {
          calls++;
    ...
    test(
      'lỗi không được phép retry (retryIf=false): dừng ngay ở lần đầu',
      () async {
        var calls = 0;
        final result = await run(() async {
          calls++;
    ...
    test(
      'hết maxAttempts vẫn lỗi: dừng đúng, không retry vô hạn, giữ cause/stackTrace cuối',
      () async {
        var calls = 0;
        final result = await run(() async {
          calls++;
    ...
    test(
      'delay/random đều đi qua hàm inject được, không dùng Future.delayed/Random thật',
      () async {
        final policy = const RetryPolicy(
          maxAttempts: 2,
          baseDelay: Duration(seconds: 999),
        );
