...
void main() {
  ...
  group('Slice 1: isFlowSeen/markFlowSeen cơ bản', () {
    test('chưa markFlowSeen: isFlowSeen trả về false, không throw kể cả chưa registerFlow', () {
    ...
    test('flowId/version rỗng hoặc <= 0 throw ArgumentError', () {
      ...
      expect(() => service.markFlowSeen('intro', version: 0), throwsArgumentError);
      expect(() => service.markFlowSeen('intro', version: -1), throwsArgumentError);
      expect(() => service.registerFlow(''), throwsArgumentError);
      expect(() => service.registerFlow('intro', version: 0), throwsArgumentError);
    });

    test('bump version cho 1 flowId đã seen ở version cũ → isFlowSeen(version mới) false đúng 1 lần, không ảnh hưởng flowId khác', () {
      final service = OnboardingCoordinatorService();
      ...
      expect(service.isFlowSeen('shop_tip', version: 1), isTrue); // không bị ảnh hưởng

      service.markFlowSeen('intro', version: 2);
      expect(service.isFlowSeen('intro', version: 2), isTrue);
    });

    test('markFlowSeen với version thấp hơn version đã ghi nhận: không "un-seen" (monotonic)', () {
      final service = OnboardingCoordinatorService();
      service.markFlowSeen('intro', version: 3);
      service.markFlowSeen('intro', version: 1); // cố tình gọi lại version cũ hơn

      expect(service.isFlowSeen('intro', version: 3), isTrue); // vẫn giữ nguyên mốc cao nhất
    });
  ...
  group('Slice 2: nextEligibleFlow — eligibility & priority', () {
    ...
    test('cùng priority: theo đúng thứ tự registerFlow được gọi (đăng ký trước chạy trước)', () {
      final service = OnboardingCoordinatorService();
    ...
    test('registerFlow lại cùng flowId (re-declare mỗi boot): cập nhật priority/version, không tự reset seen state', () {
      final service = OnboardingCoordinatorService();
    ...
    test('bump version qua registerFlow: flow đã seen ở version cũ trở thành eligible lại đúng 1 lần', () {
      final service = OnboardingCoordinatorService();
  ...
  group('Slice 3: migration & corrupt-data hardening', () {
    test('JSON hỏng (field sai kiểu, key rỗng, giá trị âm) bị lọc bỏ an toàn, không throw', () async {
    ...
    test('JSON hoàn toàn hỏng (không phải object hợp lệ): rơi về "chưa flow nào từng thấy" an toàn', () async {
