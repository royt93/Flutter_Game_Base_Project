...
void main() {
  ...
  group('build: manifest cơ bản', () {
    test(
      'có đúng schemaVersion/generatedAtMs/appVersion, section null bị bỏ qua hẳn',
      () async {
    ...
    test('4 section đều có mặt khi truyền đủ ingredient', () async {
      ...
      expect(
        sections.keys,
        containsAll(['health', 'replay', 'config', 'logs']),
      );
    });
  });
  ...
  group('build: redaction default-deny cho config', () {
    test(
      'không truyền configAllowedKeys -> config section rỗng, không leak gì',
      () async {
        final bundle = DiagnosticsExportBundle();
        final result = await bundle.build(
          appVersion: '1.0',
    ...
    test(
      'chỉ định đúng allowedKeys -> chỉ key đó lọt qua, key khác (secret) bị loại',
      () async {
        final bundle = DiagnosticsExportBundle();
        final result = await bundle.build(
          appVersion: '1.0',
    ...
    test(
      'config == null -> không có section config nào cả (khác với section rỗng)',
      () async {
        final bundle = DiagnosticsExportBundle();
  ...
  group('build: logs — tail + cap độ dài dòng', () {
    test(
      'quá maxLogLines -> chỉ giữ N dòng GẦN NHẤT (tail, không phải head)',
      () async {
        final bundle = DiagnosticsExportBundle();
  ...
  group('build: size cap — export lỗi một phần vẫn đọc được phần còn lại', () {
    test(
      'vượt maxBytes -> drop logs trước, replay/config/health vẫn còn nếu đủ nhỏ',
      () async {
        ...
        final result = await bundle.build(
          ...
          logs: List.generate(
            200,
            (i) => 'a very long log line number $i ' * 5,
          ),
        );
        ...
        final sections = result['sections'] as Map;
        expect(
          sections.containsKey('logs'),
          isFalse,
          reason: 'logs bị drop trước tiên',
        );
        expect(
          sections.containsKey('health'),
          isTrue,
          reason: 'health giữ lại lâu nhất',
        );
    ...
    test(
      'thứ tự drop cố định: logs -> replay -> config, health luôn giữ lại cuối cùng',
      () async {
  ...
  group('sign / DiagnosticsBundleView: round-trip + read-only', () {
    ...
    test(
      'sai secret -> throw FormatException, không âm thầm trả dữ liệu sai',
      () async {
        final bundle = DiagnosticsExportBundle();
