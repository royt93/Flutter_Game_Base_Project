...
void main() {
  ...
  group('parseDeepLink: pure parser', () {
    ...
    test('scheme không nằm trong allowlist: reject', () {
      final result = parseDeepLink(Uri.parse('evil://open/level/42'), _routes);
  ...
  group('DeepLinkCommandRouter: link trước app-ready', () {
    test('link đến trước markReady(): queued, handler CHƯA được gọi', () async {
      ...
      final result = await router.handleUri(Uri.parse('myapp://open/level/1'));
    ...
    test(
      'link trùng queue 2 lần trước ready: chỉ xử lý đúng 1 lần khi ready',
      () async {
        final router = DeepLinkCommandRouter(routes: _routes);
        var calls = 0;
        router.registerHandler('level', (_) => calls++);
  ...
  group('DeepLinkCommandRouter: dispatch sau khi ready', () {
    test('link hợp lệ: dispatched, handler nhận đúng command', () async {
      ...
      final result = await router.handleUri(Uri.parse('myapp://open/level/7'));
    ...
    test(
      'link không khớp route nào: rejected, không handler nào được gọi',
      () async {
        final router = DeepLinkCommandRouter(routes: _routes)..markReady();
        var calls = 0;
        router.registerHandler('level', (_) => calls++);
  ...
  group('DeepLinkCommandRouter: duplicate concurrent/relaunch', () {
    test(
      'link trùng trong cooldown: bị bỏ qua, handler chỉ chạy 1 lần',
      () async {
    ...
    test('link trùng SAU khi hết cooldown: xử lý lại bình thường', () async {
      ...
      final second = await router.handleUri(Uri.parse('myapp://open/level/1'));
    ...
    test(
      '2 lệnh gọi đồng thời (không await giữa) cùng 1 link: deterministic, chỉ 1 lần dispatch thật',
      () async {
        final router = DeepLinkCommandRouter(routes: _routes)..markReady();
        var calls = 0;
        ...
        expect(results.map((r) => r.outcome).toList(), [
          DeepLinkOutcome.dispatched,
          DeepLinkOutcome.duplicateIgnored,
        ]);
        expect(calls, 1);
      },
    );
  ...
  group('DeepLinkCommandRouter: handler throw không mất link khác', () {
    test(
      '1 handler throw: handler khác CÙNG command vẫn chạy, có diagnostic',
      () async {
        final router = DeepLinkCommandRouter(routes: _routes)..markReady();
        var secondRan = false;
    ...
    test(
      'handler của command A throw không ảnh hưởng command B khác',
      () async {
        final router = DeepLinkCommandRouter(routes: _routes)..markReady();
        ...
        await router.handleUri(Uri.parse('myapp://open/level/1'));
        final shopResult = await router.handleUri(
          Uri.parse('myapp://open/shop'),
        );
