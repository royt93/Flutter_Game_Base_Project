...
## Việc
- [x] **Trục thưởng side-mode riêng**: 1 đường "điểm side-mode/tuần" (tách hẳn Mùa giải campaign) →
  mốc thưởng xu/booster. Đọc `todayEpochDay`/`_effectiveDay` (anti-cheat), reset tuần.
- [x] **Quest xoay theo mode**: "tuần này chơi Survival đạt tầng X" / "ColorRush streak ×3" → thưởng.
  Buộc *luân phiên* mode thay vì chỉ cày 1 mode → khám phá hết bề rộng đã có.
- [x] **Mốc record sâu hơn**: trên Bronze/Silver/Gold hiện có (`side_mode_records.dart`) thêm mốc
  **Platinum + danh hiệu đeo được** (tái dùng hệ danh hiệu W18.2) — phần thưởng "khoe" không p2w.
- [x] **Liên kết Progression Tree / Battle Pass**: cho phép 1 nhánh nhỏ tiến bằng thành tích side-mode
  (nếu không phá cân bằng) → side mode "đóng góp" vào meta mà vẫn không đụng campaign progress.

## Acceptance
- [x] Trục side-mode/tuần hoạt động; thắng side mode KHÔNG đổi win-streak/level-unlock/lives (test).
- [x] Thưởng 1 lần/mốc (guard-key TRƯỚC — [[currency-persistence-convention]]); không re-claim khi
  restart; mọi key vào `resetProgress` + `resetState()` ([[reset-permanent-controllers]]).
- [x] Quest xoay đọc thời gian qua `_effectiveDay` (không `DateTime.now()` trực tiếp).
- [x] `flutter analyze` 0 · unit test (mốc 1 lần + reset + anti-cheat ngày) · suite xanh.
