# Kho ý tưởng thêm (chưa chốt) — pick khi muốn

Ý tưởng ngoài 48 task đã chốt (24 gốc + 24 đợt audit 2026-07). Nói ID để tôi
rã thành task md.
**Đã chốt và rã task** (xem `README.md`): I2, I4, I5, I11, I13, I18 (đợt đầu);
I1, I3, I6, I7, I8, I9, I10, I14, I15, I16, I17 (đợt audit 2026-07). Danh sách
dưới giữ nguyên để tra cứu mô tả gốc.

## Gameplay depth
- **I1 Gift/present tiles** ✅ đã chốt — [I1-gift-present-tiles.md](I1-gift-present-tiles.md).
- **I2 Color-lock / chain tiles** ✅ đã chốt — [I2-chain-tiles.md](I2-chain-tiles.md).
- **I3 Gravity variants** ✅ đã chốt — [I3-gravity-variants.md](I3-gravity-variants.md).
- **I4 Predictive hint** ✅ đã chốt — [I4-predictive-hint.md](I4-predictive-hint.md).
- **I5 Undo miễn phí 1 lần/màn** ✅ đã chốt — [I5-free-undo.md](I5-free-undo.md).
- **I21 Đa dạng số màu gem theo level** ✅ đã chốt — [I21-per-level-color-variety.md](I21-per-level-color-variety.md).

## Meta / retention
- **I6 Battle-pass mùa** ✅ đã chốt (free-track only) — [I6-battle-pass.md](I6-battle-pass.md).
- **I7 Spin wheel hằng ngày** ✅ đã chốt — [I7-spin-wheel.md](I7-spin-wheel.md).
- **I8 Weekend event** ✅ đã chốt — [I8-weekend-event.md](I8-weekend-event.md).
- **I9 Leaderboard bạn bè / clan co-op** ✅ đã chốt (offline giả lập) — [I9-leaderboard-friends.md](I9-leaderboard-friends.md).
- **I10 Comeback bonus** ✅ đã chốt — [I10-comeback-bonus.md](I10-comeback-bonus.md).

## Cảm giác / A-V
- **I11 Haptic feedback** ✅ đã chốt — [I11-haptic-feedback.md](I11-haptic-feedback.md).
- **I12 Dynamic music layers** — nhạc thêm lớp khi combo cao. *(chưa chốt)*
- **I13 SFX pop cao độ theo cỡ nhóm** ✅ đã chốt — [I13-pop-sfx-pitch.md](I13-pop-sfx-pitch.md).
- **I14 Theme per world** ✅ đã chốt — [I14-theme-per-world.md](I14-theme-per-world.md).

## Neon/Glow độc quyền
- **I15 Day/Night theme toggle** ✅ đã chốt — [I15-day-night-toggle.md](I15-day-night-toggle.md).
- **I16 Aurora background shader** ✅ đã chốt — [I16-aurora-background-shader.md](I16-aurora-background-shader.md).
- **I17 Tile material variants** ✅ đã chốt — [I17-tile-material-variants.md](I17-tile-material-variants.md).
- **I18 Colorblind neon symbols** ✅ đã chốt — [I18-colorblind-symbols.md](I18-colorblind-symbols.md).

## Ý tưởng độc quyền khác (đã chốt, thuộc nhóm Features)
- **Relic/Perk system** ✅ đã chốt (không pay-to-win) — [F14-relic-perk-system.md](F14-relic-perk-system.md).
- **Photo mode / chia sẻ bàn chơi** ✅ đã chốt — [F15-photo-mode-share.md](F15-photo-mode-share.md).

## Monetize (nếu cần — vẫn ngoài scope, user đã loại)
- **I19 Coin doubler / starter pack** (IAP). *(chưa chốt, ngoài scope)*
- **I20 Rewarded continue** — xem quảng cáo để +5 nước khi thua (nếu bật ad lại). *(chưa chốt, ngoài scope)*
