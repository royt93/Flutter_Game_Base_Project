...
## Acceptance criteria
- [ ] Chọn 1 hướng, ghi rõ trong `## Quyết định`.
- [ ] `generateConfettiParticles`/`ConfettiParticle`'s pure functions (`confettiOffsetAt`, `confettiRotationAt`, `confettiOpacityAt`) vẫn giữ nguyên chữ ký, không phá test hiện có (`test/widget/common/confetti_overlay_test.dart`).
- [ ] Test mới cho phần thêm (shape hoặc gradient) nếu logic đủ phức tạp để cần test riêng.
