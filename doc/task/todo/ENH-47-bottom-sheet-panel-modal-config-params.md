---
id: ENH-47
title: "showCommonBottomSheet: thiếu isDismissible/enableDrag/useRootNavigator/barrierColor"
type: enhancement
priority: P3
effort: S
...
## Vị trí
`lib/presentation/widgets/common/bottom_sheet_panel.dart` — `showCommonBottomSheet`.

## Hiện trạng
Hàm helper không expose các tham số cấu hình modal tiêu chuẩn của `showModalBottomSheet` (`isDismissible`, `enableDrag`, `useRootNavigator`, `barrierColor`) — caller cần các hành vi này (ví dụ 1 sheet bắt buộc phải chọn, không cho vuốt tắt) phải tự viết lại toàn bộ helper thay vì dùng bản có sẵn.

## Vì sao cần / Hậu quả
Hạn chế tái sử dụng cho các luồng cần kiểm soát chặt hành vi dismiss (ví dụ sheet xác nhận quan trọng).
