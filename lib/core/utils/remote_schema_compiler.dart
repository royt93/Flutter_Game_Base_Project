...
SaveMigrationRegistry buildMigrationRegistry(
  ...
  for (var i = 0; i < schema.versions.length - 1; i++) {
    ...
    steps.add(
      SaveMigrationStep(fromVersion: from, toVersion: to, migrate: migrate),
    );
  }
  return SaveMigrationRegistry(
    currentVersion: schema.current.version,
    steps: steps,
  );
}
