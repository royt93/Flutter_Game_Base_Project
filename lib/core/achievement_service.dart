...
class AchievementService extends GetxService {
  ...
  // existing tests below construct a second instance and read it back
  // with NO `await` at all between that and the save-triggering call,
  // relying on the FIRST save of a burst starting (and, for the in-memory
  // fallback `StorageService` these tests use, completing) synchronously.
  // `.then()` (and an `await` INSIDE another async function's own body
  // awaiting a nested async call — also tried, also confirmed broken the
  // same way) is ALWAYS deferred via `scheduleMicrotask`, even on an
  // already-complete Future — only a plain, un-awaited direct call to an
  // async function whose own body never actually suspends runs fully
  // synchronously. So the fix below calls `_runSave()` directly (never
  // wrapped in `await` from another async function), and reschedules
  // itself via `Future.whenComplete` instead of looping inside an `async`
  // method.
  //
