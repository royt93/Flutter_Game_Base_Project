...
LOG_FILE="${TMPDIR:-/tmp}/neon_jewels_device_gate_${SERIAL}_$(date +%Y%m%d_%H%M%S).log"
...
ERROR_PATTERN='FATAL EXCEPTION|AndroidRuntime|ANR in|E/flutter|Failed assertion|RenderFlex overflowed|Process .*crashed|Fatal signal|crash_dump.*com\.galaxyjoy\.neon_jewels'

if grep -E "$ERROR_PATTERN" "$LOG_FILE" >/tmp/neon_jewels_device_gate_matches.txt; then
  echo "ERROR: device gate found fatal log lines:" >&2
  cat /tmp/neon_jewels_device_gate_matches.txt >&2
  exit 1
fi
